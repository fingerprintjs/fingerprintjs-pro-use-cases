...
/**
 * Server env schema with no `server-only` marker.
 * Use from Node loaders (next.config.ts). App/server code should import `./server` instead
 * so a client import fails the Next compile.
 *
